...
function cargarDatosGasto(id) {
    fetch(`http://localhost:8080/gastos/${id}`, {
        ...
        .then(res => res.json())
        .then(gastos => {
            document.getElementById('importeGasto').value = gastos.concepto;
            document.getElementById('nombreConcepto').value = gastos.tipo;
            document.getElementById('mesGasto').value = gastos.mes;
            document.getElementById('anioGasto').value = gastos.anio;
            document.getElementById('fechaVencimientoGasto').value = gastos.fecha_vencimiento;
            document.getElementById('fechaVencimientoDiv').checked = gastos.requiere_vencimiento == 1;
